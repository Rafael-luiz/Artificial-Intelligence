...
# Criando o DataSet =======================================================
reviews_test_full = []
labels_reviews_test_full = []

reviews_test_json = json.load(open('reviews/reviews_test.json', 'r', encoding="utf8"))

reviews_test_pos = reviews_test_json['positivas']
labels_reviews_test_pos = []

reviews_test_neg = reviews_test_json['negativas']
labels_reviews_test_neg = []

for review in reviews_test_pos:
    reviews_test_full.append(review.strip())
...
for label_pos in labels_reviews_test_pos:
    labels_reviews_test_full.append(label_pos)

for label_neg in labels_reviews_test_neg:
    labels_reviews_test_full.append(label_neg)

# Criando a RNA ==========================================================
onehot_enc = MultiLabelBinarizer()
onehot_enc.fit(labels_reviews_test_full)

X_train, X_test, y_train, y_test = train_test_split(labels_reviews_test_full, labels_reviews_test_full, test_size=0.4, random_state=None)
...
def label2bool(labels):
    return [[1, 0] if label == "positiva" else [0, 1] for label in labels]
...
#pprint(labels_reviews_test_full)
#pprint(reviews_test_full)
