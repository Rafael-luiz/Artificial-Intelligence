...
def calculates_results_stats(results_dic):
    ...
    n_dogs_img = n_notdogs_img = n_match = n_correct_dogs = n_correct_notdogs = n_correct_breed = n_labels_match = 0       
    
    results_stats_dic = {'n_images': n_images, 'n_dogs_img': 0, 
                         'n_notdogs_img': 0, 'n_match': 0, 
                         'n_correct_dogs': 0, 'n_correct_notdogs': 0, 
                         'n_correct_breed': 0, 'n_labels_match': 0 }  
    ...
    for key, value in results_dic.items():     
        ...
        # n_labels_match - number of label matches
        if results_dic[key][2] == 1:
            n_labels_match += 1
            results_stats_dic.update({'n_labels_match': n_labels_match})

    
    # pct_match - percentage of correct matches
    pct_macth = (n_dogs_img/n_images)*100
    ...
    # pct_correct_label_match - Percentage of correctly Matched Images
    pct_correct_label_match = (n_labels_match/n_images)*100
    
    results_stats_dic.update({'pct_macth': pct_macth,  
                              'pct_correct_dogs': pct_correct_dogs,
                              'pct_correct_breed': pct_correct_breed,
                              'pct_correct_notdogs': pct_correct_notdogs, 
                              'pct_correct_label_match': pct_correct_label_match})
